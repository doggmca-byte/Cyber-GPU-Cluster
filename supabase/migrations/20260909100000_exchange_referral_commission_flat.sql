-- =====================================================================================
-- Реферальна комісія з обміну HASH→TON: ФІКСОВАНІ 0.01 TON, один раз на реферала.
--
-- Уточнення до 20260909090000_exchange_referral_commission_once.sql: там комісія
-- лишалась пропорційною обсягу першого обміну (0.01 × кожні 1000 HASH), тож один
-- великий перший обмін міг дати рефереру відразу пів TON. Тепер сума завжди рівно
-- 0.01 TON — незалежно від того, скільки HASH реферал обміняв.
--
-- Решта поведінки без змін: платимо на ПЕРШОМУ обміні реферала, прапорець
-- referrals.exchange_commission_paid і далі є замком від повторних/паралельних
-- нарахувань, бекфіл уже виконано попередньою міграцією.
--
-- Текст в інтерфейсі НЕ змінюється — за прямим рішенням продукту.
-- =====================================================================================
...
declare
    ...
    -- Фіксована разова сума на реферала — НЕ множиться на обсяг обміну.
    v_referral_commission_once   constant numeric(18, 6) := 0.01;
...
begin
    ...
    -- 2) Реферальна комісія — фіксовані 0.01 TON, ОДИН раз на реферала,
    -- на його першому обміні.
    ...
    if v_referrer_id is not null then
        v_commission := v_referral_commission_once;
