-- =====================================================================================
-- 20 переглядів реклами перед КОЖНИМ виводом.
--
-- Ця вимога вже була (MIN_ADS_BEFORE_WITHDRAW = 20), але 19.08 її жорстку
-- перевірку прибрали, і з того часу лічильник лише малювався прогрес-баром:
-- вивести можна було, не подивившись жодної реклами. Повертаємо як правило.
--
-- Окремий лічильник не потрібен: ads_watched_since_withdraw і так обнуляється
-- при кожній успішній заявці, тож перевірка "не менше 20" на момент заявки =
-- "20 реклам перед кожним виводом". Решта функції — без змін.
-- =====================================================================================

CREATE OR REPLACE FUNCTION public.request_withdrawal(p_user_id uuid, p_amount numeric, p_destination_address text)
 RETURNS TABLE(transaction_id uuid, requested_amount numeric, fee_charged numeric, net_payout numeric, destination_address text, withdrawable_balance numeric, withdrawal_quota numeric, ads_watched_since_withdraw integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
    ...
    v_min_ads_per_withdrawal     constant integer  := 20;    -- MIN_ADS_BEFORE_WITHDRAW, перед КОЖНОЮ заявкою
...
begin
    ...
    -- 20 переглядів реклами перед КОЖНИМ виводом. Лічильник обнуляється
    -- при кожній успішній заявці (нижче, ads_watched_since_withdraw = 0), тож
    -- ця перевірка і є "20 реклам щоразу". Стоїть другою — після депозиту й
    -- до балансу: це перша дія, яку гравець може зробити сам, тож і
    -- повідомлення має бути саме про неї, а не про квоту чи суму.
    if coalesce(v_profile.ads_watched_since_withdraw, 0) < v_min_ads_per_withdrawal then
        raise exception 'watch at least % ads before each withdrawal (watched %)',
            v_min_ads_per_withdrawal, coalesce(v_profile.ads_watched_since_withdraw, 0)
            using errcode = 'P0001';
    end if;
