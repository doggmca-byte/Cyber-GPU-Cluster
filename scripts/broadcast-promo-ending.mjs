/**
 * Розсилка-нагадування: акція -10% ось-ось завершиться.
 * Інфраструктура (черга, ліміт швидкості, 403 -> is_bot_blocked, пагінація)
 * спільна з broadcast-promo.mjs — тут інші лише тексти.
 *
 * Запуск:
...
const MESSAGES = {
  en: (n, rigs) => ({
    text: `⏳ The 10% discount ends in 30 minutes.

After that these go back to full price:
${rigs}`,
    button: "Open Market",
  }),
  ru: (n, rigs) => ({
    text: `⏳ Скидка 10% заканчивается через 30 минут.

После этого вернутся к полной цене:
${rigs}`,
    button: "Открыть маркет",
  }),
  uk: (n, rigs) => ({
    text: `⏳ Знижка 10% закінчується через 30 хвилин.

Після цього повернуться до повної ціни:
${rigs}`,
    button: "Відкрити маркет",
  }),
  es: (n, rigs) => ({
    text: `⏳ El descuento del 10% termina en 30 minutos.

Después vuelven a su precio completo:
${rigs}`,
    button: "Abrir mercado",
  }),
  ar: (n, rigs) => ({
    text: `⏳ ينتهي خصم 10% خلال 30 دقيقة.

بعدها تعود إلى سعرها الكامل:
${rigs}`,
    button: "افتح المتجر",
  }),
  id: (n, rigs) => ({
    text: `⏳ Diskon 10% berakhir dalam 30 menit.

Setelah itu kembali ke harga penuh:
${rigs}`,
    button: "Buka Market",
  }),
  kk: (n, rigs) => ({
    text: `⏳ 10% жеңілдік 30 минуттан кейін аяқталады.

Одан кейін толық бағаға оралады:
${rigs}`,
    button: "Дүкенді ашу",
  }),
  tr: (n, rigs) => ({
    text: `⏳ %10 indirim 30 dakika içinde bitiyor.

Sonrasında tam fiyata dönüyor:
${rigs}`,
    button: "Marketi aç",
  }),
};
