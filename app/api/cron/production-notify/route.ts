 ...
 *
 * Крон ходить кожні дві години з 06:00 до 20:00 UTC (vercel.json). Досі це
 * була одна перевірка на добу — стеля Hobby-плану, через яку людина з
 * зупиненим майнінгом дізнавалась про це в середньому за пів доби. Тепер
 * затримка — близько години. Нічні години UTC навмисно пропущені: точність
 * до хвилини тут нікому не потрібна ціною повідомлення о третій ночі.
 *
 * Це все одно не "рівно через 12г", а перевірка "хто зараз призупинений і
 * кому ще не надсилали за цей епізод" (supabase/migrations/
 * 20260821100000_production_paused_notifications.sql) — прогін ідемпотентний,
 * тож частіші запуски не перетворюються на повторні повідомлення.
 *
 ...
 * 5xx, rate-limit) лишають користувача "не сповіщеним", і наступний прогін
