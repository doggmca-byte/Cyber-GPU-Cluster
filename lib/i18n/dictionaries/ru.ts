import type { TranslationDictionary } from "./en";

const ru: TranslationDictionary = {
  common: {
    close: "Закрыть",
    cancel: "Отмена",
    loading: "Загрузка...",
    retry: "Попробовать снова",
    comingSoon: "Скоро",
    unknownError: "Неизвестная ошибка",
    ton: "TON",
    hash: "HASH",
  },
  nav: {
    wallet: "Кошелёк",
    exchange: "Обмен",
    farm: "Ферма",
    market: "Маркет",
    friends: "Друзья",
  },
  support: {
    label: "Поддержка",
  },
  notifications: {
    productionPaused: () =>
      "Хранилище фермы заполнено! Зайдите в Cyber GPU Cluster, чтобы забрать $HASH и продолжить майнинг.",
    specialTasksAnnouncement:
      "🎁 Появились новые Особые задания! Добавь наш тег в имя или свою реферальную ссылку в Bio и удерживай через 6 этапов (15 мин → 30 дней) — получи до 0.232 TON за задание. Открой приложение → Задания → ⭐ Особые, чтобы начать.",
    openAppButton: "Открыть Cyber GPU Cluster",
    dailyBonusReady: `🎁 Ежедневный бонус готов.\n\nЗабрать — одно касание, а серия продолжает расти.`,
    tasksWaiting: (count: number, reward: string) =>
      `✅ У тебя ${count} невыполненных заданий — всего до ${reward} TON.\n\nБольшинство занимает меньше минуты.`,
    welcomeTitle: "⚡ Кластер ждёт своего оператора!",
    welcomeBody:
      "Запусти GPU Cluster, добывай $HASH, качай мощность фермы и выводи TON.\n\nСобери команду по рефералке — больше друзей, больше бонусов.\n\nЗаходи и запускай майнинг — цикл уже пошёл!",
    welcomeLaunchButton: "🚀 Запустить игру",
    welcomeCommunityButton: "📢 Вступить в сообщество",
    welcomeChatButton: "💬 Присоединиться к чату",
  },
  intro: {
    title: "Cyber GPU Cluster",
    statusInit: "Инициализация узлов GPU Cluster...",
    statusConnect: "Подключение к TON Blockchain и Supabase...",
    statusSync: "Синхронизация хешрейта и майнинг-метрик...",
    statusReady: "Готово! Запускаем интерфейс...",
    connectionIssueTitle: "Проблема с соединением",
    connectionIssueDescription: "Не удалось связаться с сервером. Проверь интернет и попробуй ещё раз.",
    retryButton: "Попробовать снова",
  },
  noTelegram: {
    title: "Открой через Telegram",
    description: "Это приложение работает только как Telegram Mini App. Открой бота и нажми кнопку запуска.",
  },
  syncError: {
    title: "Ошибка синхронизации",
  },
  language: {
    pickerTitle: "Выбери язык",
  },
  rarity: {
    common: "Обычная",
    uncommon: "Необычная",
    rare: "Редкая",
    elite: "Элитная",
    epic: "Эпическая",
    legendary: "Легендарная",
    mythic: "Мифическая",
    ancient: "Древняя",
    divine: "Божественная",
    transcendent: "Трансцендентная",
  },
  farm: {
    dailyBonus: "Daily Bonus",
    totalPower: "Общая мощность",
    hashPerHourSuffix: "HASH/ч",
    accumulatedHash: "Накоплено $HASH",
    harvestButton: "Собрать HASH",
    harvesting: "Собираем...",
    emptyGpuList: "У тебя пока нет GPU. Загляни на Market, чтобы начать майнить.",
    activeServers: "Активные серверы",
    buyNewServer: "Купить новый сервер",
    uptime: "Время работы",
    productionPaused: "Производство приостановлено — собери HASH, чтобы продолжить",
    gpuDead: "Ресурс исчерпан",
    reviveCount: (count, max) => `Оживлений: ${count}/${max}`,
    reviveButton: (cost) => `Оживить · ${cost} TON`,
    reviving: "Оживляем...",
    reviveMaxReached: "Ресурс окончательно исчерпан",
    unitsLabel: "Юниты",
    resourceLabel: "Ресурс",
    liveLabel: "В сети",
  },
  writeAccess: {
    title: "Разреши боту писать тебе!",
    description: "Чтобы не пропустить важные обновления (новые задания, бонусы, статус вывода) — разреши Cyber GPU Cluster присылать тебе сообщения.",
    allowButton: "Разрешить",
    laterButton: "Не сейчас",
  },
  market: {
    gameBalance: "Игровой баланс",
    owned: (amount, max) => `Куплено ${amount}/${max}`,
    hashPerHour: (value) => `${value} HASH/ч`,
    hashPerDay: (value) => `${value} HASH/день`,
    buy: (price) => `Купить · ${price} TON`,
    buying: "Покупаем...",
    limitReached: "Лимит достигнут",
    notEnough: (amount: string) => `Не хватает ${amount} TON`,
    reviveOnFarm: "Оживи на Ферме, чтобы снова покупать",
    promo: {
      banner: (percent) => `-${percent}% на избранные риги`,
      anniversaryBanner: (percent) => `🎉 1 Месяц Cyber GPU Cluster! Скидка ${percent}% на всё оборудование`,
      endsIn: (days, time) => `До конца акции: ${days} дн. ${time}`,
    },
    cycles: {
      title: (name) => `Циклы: ${name}`,
      quantityLabel: "Количество",
      goalPerCycle: (amount) => `Цель цикла: ${amount} TON`,
      durationPerCycle: (days) => `Длительность цикла: ~${days} дн.`,
      productionPerHour: (amount) => `${amount} TON/ч`,
      cycleRow: (n, cost, roi, profit) =>
        `Цикл ${n} · стоимость ${cost} TON · ROI ~${roi} дн. · прибыль ${profit} TON`,
      totalCost: "Общие расходы",
      totalProduction: "Общее производство",
      netProfit: "Чистая прибыль",
      openButton: "Циклы",
    },
  },
  exchange: {
    hashToTon: {
      title: "$HASH → TON",
      rateNote: (min) => `Курс: 100 000 HASH = 1 TON · минимум ${min} HASH`,
      available: "Доступно",
      amountPlaceholder: (min) => `Минимум ${min}`,
      minAmountError: (min) => `Минимальная сумма — ${min} HASH`,
      stepError: (step) => `Сумма должна быть кратна ${step} HASH.`,
      targetWithdrawable: "Withdrawable",
      targetGame: "Game",
      grossRow: "Получишь (брутто)",
      feeRow: "Комиссия",
      netRow: "К зачислению",
      submit: "Обменять",
      submitting: "Обмениваем...",
      success: (amount) => `Зачислено ${amount} TON`,
    },
    convertBack: {
      title: "Withdrawable → Game",
      rateNote: (min) => `Курс 1:1 · минимум ${min} TON`,
      withdrawableLabel: "Withdrawable",
      gameLabel: "Game",
      amountPlaceholder: (min) => `Минимум ${min}`,
      minAmountError: (min) => `Минимальная сумма — ${min} TON`,
      submit: "Перевести",
      submitting: "Переводим...",
      success: (amount) => `Переведено ${amount} TON в игровой баланс`,
    },
  },
  friends: {
    invitedFriends: "Приглашено друзей",
    totalEarned: "Заработано всего",
    pendingCommission: "Pending Commission",
    claimButton: "Забрать всё",
    claiming: "Забираем...",
    claimSuccess: (amount) => `Зачислено ${amount} TON`,
    rulesTitle: "Как начисляется реферальное вознаграждение",
    ruleRevshare: (percent) => `${percent}% с каждого депозита друга — навсегда`,
    ruleExchangeCommission: (bonus, step) =>
      `${bonus} TON реферальной комиссии за каждые ${step} HASH, которые друг обменивает/продаёт на бирже`,
    yourLink: "Твоя реферальная ссылка",
    share: "Поделиться",
    notConfigured: "NEXT_PUBLIC_TELEGRAM_BOT_USERNAME не настроен.",
    shareMessage: "Присоединяйся к Cyber GPU Cluster и добывай $HASH вместе со мной!",
  },
  wallet: {
    gameBalance: "Game Balance",
    withdrawable: "Withdrawable",
    quota: "Квота вывода",
    adsWatched: (watched, required) =>
      `Реклам просмотрено: ${watched}/${required} с момента последнего вывода`,
    depositButton: "Пополнить",
    withdrawButton: "Вывести",
    deposit: {
      title: "Пополнить Game Balance",
      notConfigured: "Приём депозитов ещё не настроен (NEXT_PUBLIC_TREASURY_TON_ADDRESS отсутствует).",
      creditedTitle: "Зачислено!",
      creditedAmount: (amount) => `+${amount} TON на Game Balance`,
      done: "Готово",
      chooseAmount: "Выбери сумму пополнения в TON",
      confirmInWallet: "Подтверди в кошельке...",
      verifyingOnChain: "Проверяем в блокчейне...",
      depositAmount: (amount) => `Пополнить на ${amount} TON`,
      pickAmount: "Выбери сумму",
      customAmountLabel: (min) => `Или введи свою сумму (мин. ${min} TON)`,
      customAmountPlaceholder: "напр. 0.5",
      minAmountError: (min) => `Минимальная сумма пополнения — ${min} TON`,
      minDepositNote: (min) => `Рекомендуемая минимальная сумма депозита — ${min} TON.`,
      disclaimer: "Средства зачисляются только после подтверждения транзакции в сети TON.",
      timeoutError: "Транзакция не подтвердилась вовремя. Попробуй кнопку «Проверить оплату» чуть позже.",
      addressLabel: "Адрес кошелька проекта",
      memoLabel: "Комментарий / Memo (твой Telegram ID)",
      memoWarning: "Обязательно укажи этот комментарий при переводе, иначе депозит не зачислится автоматически!",
      checkButton: "Проверить оплату",
      checking: "Проверяем...",
      checkNotFound: "Новый платёж пока не найден. Если ты уже отправил TON — подожди немного и попробуй снова.",
      orQuickPay: "или оплати мгновенно",
    },
    withdraw: {
      title: "Вывести TON",
      watchAdsPrompt: (watched, required) => `Посмотри рекламу: ${watched}/${required}`,
      balanceLabel: "Withdrawable Balance",
      quotaLabel: "Квота вывода",
      addressLabel: "Адрес TON-кошелька получателя",
      addressPlaceholder: "UQ... / EQ...",
      invalidAddress: "Невалидный TON-адрес.",
      amountLabel: "Сумма",
      amountPlaceholder: "Сумма в TON",
      requestRow: "Запрос",
      feeRow: (percent) => `Комиссия (${percent}%)`,
      netRow: "К выплате",
      insufficientBalance: "Недостаточно средств на Withdrawable Balance.",
      insufficientQuota: "Превышена квота вывода — посмотри больше рекламы, чтобы разблокировать.",
      feeRowFlat: (amount) => `Комиссия (${amount} TON)`,
      minTierHint: (min) => `Минимум для этого вывода: ${min} TON`,
      maxTierHint: (max) => `Максимум за заявку сейчас: ${max} TON`,
      oneRequestPerDay: "Разрешена только 1 заявка в сутки (сброс в 00:00 UTC).",
      depositRequired: (minDeposit) =>
        `Ты уже использовал единственный вывод без депозита. Чтобы вывести снова, пополни минимум на ${minDeposit} TON.`,
      freeWithdrawalHint: (amount, minDeposit) =>
        `Первый вывод без депозита — ровно ${amount} TON. Дальше для вывода нужно пополнение минимум на ${minDeposit} TON.`,
      submit: "Вывести",
      submitting: "Отправляем заявку...",
      success: (amount, addressShort) =>
        `Заявка создана. К выплате ${amount} TON на ${addressShort} (статус: ожидает обработки).`,
    },
    history: {
      toggleShow: "Показать историю выводов",
      toggleHide: "Скрыть историю выводов",
      title: "История выводов",
      empty: "Заявок на вывод ещё не было.",
      loading: "Загрузка...",
      loadError: "Не удалось загрузить историю выводов.",
      statusPending: "Ожидает обработки",
      statusProcessing: "Отправляется...",
      statusCompleted: "Выполнено",
      statusRejected: "Отклонено",
      statusFailed: "Ошибка",
      statusCancelled: "Отменено",
      reason: (reason) => `Причина: ${reason}`,
      netAmount: (amount) => `К выплате: ${amount} TON`,
    },
  },
  watchAd: {
    button: "Посмотреть рекламу",
    loading: "Загрузка рекламы...",
    adNotCompleted: "Реклама не была просмотрена до конца. Попробуй ещё раз.",
    confirming: "Подтверждаем просмотр...",
    notCounted: "Просмотр не засчитан — попробуй ещё раз.",
    stillProcessing: "Ещё обрабатывается — проверь баланс чуть позже.",
  },
  errorPage: {
    title: "Что-то пошло не так",
    description:
      "Произошла непредвиденная ошибка. Попробуй ещё раз — если проблема повторяется, перезапусти приложение через Telegram.",
    retry: "Попробовать снова",
  },
  tasks: {
    openButton: "Задания",
    title: "Центр Заданий",
    subtitle: "Выполняй задания и получай награды",
    progress: (done, total) => `⚡ ${done}/${total} ВЫПОЛНЕНО`,
    empty: "В этой категории пока нет заданий.",
    categories: {
      in_game: "В игре",
      general: "Общие",
      partners: "Партнёры",
      wallet: "Кошелёк",
      friends: "Друзья",
      special: "Особые",
    },
    reward: {
      game_balance: (amount) => `+${amount} TON (Игра)`,
      ton: (amount) => `+${amount} TON`,
      quota: (amount) => `+${amount} Квоты`,
    },
    action: {
      start: "Выполнить",
      verify: "Проверить",
      verifying: "Проверяем...",
      claim: "Забрать",
      claiming: "Забираем...",
      claimed: "✓ Получено",
      notCompletedYet: "Ещё не подтверждено — попробуй снова после выполнения действия.",
      awaitingPartner: "Ожидаем подтверждения от партнёра...",
    },
    items: {
      own_gpus_5: {
        title: "Владей 5 GPU",
        description: "Купи и владей минимум 5 единицами GPU одновременно.",
      },
      own_gpus_20: {
        title: "Владей 20 GPU",
        description: "Развей ферму до 20 единиц GPU одновременно.",
      },
      harvest_10: {
        title: "Собери HASH 10 раз",
        description: "Нажми кнопку сбора HASH 10 раз.",
      },
      harvest_50: {
        title: "Собери HASH 50 раз",
        description: "Нажми кнопку сбора HASH 50 раз.",
      },
      subscribe_channel: {
        title: "Подпишись на наш канал",
        description: "Присоединяйся к официальному Telegram-каналу с новостями и обновлениями.",
      },
      subscribe_chat: {
        title: "Присоединяйся к чату",
        description: "Присоединяйся к официальному групповому чату, чтобы общаться с сообществом.",
      },
      subscribe_payouts_channel: {
        title: "Подпишись на канал выплат",
        description: "Присоединяйся к каналу, где публикуются все выплаты.",
      },
      visit_website: {
        title: "Посети наш сайт",
        description: "Открой сайт Cyber GPU Cluster.",
      },
      partner_channel: {
        title: "Присоединись к партнёрскому каналу",
        description: "Подпишись на Telegram-канал нашего партнёра.",
      },
      partner_offer: {
        title: "Посмотри предложение партнёра",
        description: "Открой страницу предложения партнёра.",
      },
      cookie_wars_game_enter: {
        title: "Cookie Wars",
        description: "Открой приложение Cookie Wars.",
      },
      crybble_game_enter: {
        title: "Crybble",
        description: "Зайди на арену Crybble за 1 CBW (подарок).",
      },
      cookie_hunters_signup: {
        title: "Cookie Hunters",
        description: "+$3 на бонусный счёт просто за регистрацию — стреляй и зарабатывай.",
      },
      dogs_house_miner_start: {
        title: "Dogs House Miner",
        description: "Запусти бота и начни майнить GRAM.",
      },
      mrg_miner_start: {
        title: "MRG Miner",
        description: "Открой приложение и начни майнить.",
      },
      shares_your_links_chat: {
        title: "Чат «Обмен рефералами»",
        description: "Вступи в чат и делись своими реферальными ссылками бесплатно.",
      },
      axvr_stars_miner_start: {
        title: "AXVR Stars Miner",
        description: "Открой бота и начни майнить Stars.",
      },
      coin_kero_start: {
        title: "COIN KERO | BOT",
        description: "Начни майнить в COIN KERO.",
      },
      theiterra_enter: {
        title: "TheiTerra",
        description: "Открой TheiTerra и создай профиль.",
      },
      gram_generator_enter: {
        title: "Gram Generator",
        description: "Открой Gram Generator и попробуй его.",
      },
      minerias_channel: {
        title: "MINERIAS Y BOT",
        description: "Подпишись на канал MINERIAS Y BOT.",
      },
      crypto_hunters_channel: {
        title: "Crypto Hunters",
        description: "Подпишись на канал Crypto Hunters.",
      },
      zoomverse_join: {
        title: "ZoomVerse",
        description: "Присоединяйся к Zoom и зарабатывай $ZOOM!",
      },
      atf_token_join: {
        title: "ATF Token",
        description: "Открой ATF Token и получи airdrop.",
      },
      starnex_win_combinations: {
        title: "StarNex",
        description: "Сыграй в StarNex и выбей 2 выигрышные комбинации.",
      },
      memefarm_open: {
        title: "MEME FARM",
        description: "Открой MEME FARM и создай свой профиль.",
      },
      first_deposit: {
        title: "Сделай первый депозит",
        description: "Пополни Game Balance реальным TON хотя бы один раз.",
      },
      deposit_5: {
        title: "Депозит 5 TON",
        description: "Пополни Game Balance на 5 TON суммарно — любым количеством депозитов.",
      },
      deposit_10: {
        title: "Депозит 10 TON",
        description: "Пополни Game Balance на 10 TON суммарно — любым количеством депозитов.",
      },
      deposit_25: {
        title: "Депозит 25 TON",
        description: "Пополни Game Balance на 25 TON суммарно — любым количеством депозитов.",
      },
      deposit_50: {
        title: "Депозит 50 TON",
        description: "Пополни Game Balance на 50 TON суммарно — любым количеством депозитов.",
      },
      deposit_100: {
        title: "Депозит 100 TON",
        description: "Пополни Game Balance на 100 TON суммарно — любым количеством депозитов.",
      },
      deposit_500: {
        title: "Депозит 500 TON",
        description: "Пополни Game Balance на 500 TON суммарно — любым количеством депозитов.",
      },
      deposit_1000: {
        title: "Депозит 1000 TON",
        description: "Пополни Game Balance на 1000 TON суммарно — любым количеством депозитов.",
      },
      invite_1: {
        title: "Пригласи 1 друга",
        description: "Пригласи друга по своей реферальной ссылке.",
      },
      invite_5: {
        title: "Пригласи 5 друзей",
        description: "Пригласи 5 друзей по своей реферальной ссылке.",
      },
      invite_20: {
        title: "Пригласи 20 друзей",
        description: "Пригласи 20 друзей по своей реферальной ссылке.",
      },
      special_promo: {
        title: "Бонус на ограниченное время",
        description: "Особая награда, доступная ограниченное время.",
      },
      harvest_1000_hash: {
        title: "Собери 1000 HASH суммарно",
        description: "Накопи 1000 $HASH за всё время игры — любым количеством сборов.",
      },
      own_legendary_gpu: {
        title: "Получи легендарную GPU",
        description: "Приобрети NVIDIA A100 Substation — карту уровня Legendary.",
      },
    },
    partnerAds: {
      title: "Смотри рекламу партнёров",
      description: "Смотри рекламу партнёров и получай TON за каждый просмотр.",
      button: "Смотреть рекламу",
      loading: "Загрузка рекламы...",
      adNotCompleted: "Реклама не досмотрена до конца. Попробуй ещё раз.",
      progress: (watched, limit) => `${watched}/${limit} сегодня`,
      limitReached: "Дневной лимит исчерпан — возвращайся завтра.",
      confirming: "Подтверждаем просмотр...",
      notCounted: "Просмотр не засчитан — попробуй ещё раз.",
      stillProcessing: "Ещё обрабатывается — проверь баланс чуть позже.",
      tadsTurnHint: "Сейчас очередь спонсорского баннера — нажми на баннер ниже, чтобы посмотреть его.",
    },
    tadsAd: {
      title: "Спонсорский баннер",
      description: "Нажми на баннер ниже и получи TON.",
      noAdsAvailable: "Сейчас нет доступных спонсорских баннеров.",
    },
  },
  dailyBonus: {
    modalTitle: "Daily Bonus",
    loading: "Загрузка...",
    loadError: "Не удалось загрузить статус ежедневного бонуса.",
    cooldownTitle: "Возвращайся завтра!",
    cooldownLabel: (time) => `⏱️ До следующей награды: ${time}`,
    adScreenTitle: "Перед просмотром",
    adScreenSubtitle: "Реклама запустится автоматически — награда зачислится сразу после просмотра.",
    checklist: {
      watchFull: "Досмотрите рекламу до конца.",
      twoButtons: "Нажмите минимум на 2 разные рекламные кнопки.",
      stay5s: "Оставайтесь на целевой странице минимум 5 секунд.",
    },
    adStartingIn: (seconds) => `⏱️ Реклама начнётся через ${seconds} сек...`,
    adInProgress: "⏱️ Показываем рекламу...",
    cancel: "Отмена",
    claimButton: (amount) => `Проверить / Забрать ${amount} TON`,
    claiming: "Проверяем...",
    claimError: "Не удалось забрать награду. Попробуй ещё раз.",
    noAdAvailable: "Сейчас нет доступной рекламы. Попробуйте позже.",
    cooldownActiveError: "Кулдаун ещё активен.",
    confirming: "Подтверждаем просмотр...",
    notCounted: "Просмотр не засчитан — попробуй ещё раз.",
    stillProcessing: "Ещё обрабатывается — проверь баланс чуть позже.",
    claimedTitle: "Награда получена!",
    claimedAmount: (amount) => `+${amount} TON зачислено`,
    done: "Готово",
  },
  retentionTasks: {
    sectionTitle: "Особые задания",
    sectionSubtitle: "Удерживай условие все 6 этапов — награда растёт с каждым этапом.",
    nameTagTitle: "Добавь наш тег в имя",
    nameTagDescription: "Добавь текст ниже в имя или фамилию Telegram.",
    bioLinkTitle: "Добавь реферальную ссылку в Bio",
    bioLinkDescription: "Добавь свою персональную реферальную ссылку ниже в Bio Telegram.",
    copyLabel: "Нужный текст",
    copyButton: "Копировать",
    copied: "Скопировано!",
    rewardProgress: (current, total, unit) => `${current} / ${total} ${unit}`,
    stageLabel: (n) => `Этап ${n}`,
    unitMinutes: (n) => `${n} мин`,
    unitHours: (n) => `${n} ч`,
    unitDays: (n) => `${n} дн`,
    startButton: "Старт",
    starting: "Запускаем...",
    verifyButton: "Проверить",
    verifying: "Проверяем...",
    readyToVerify: "Готово — жми «Проверить»!",
    countdownLabel: (time) => `Следующая проверка через ${time}`,
    notStartedHint: "Добавь это выше, затем нажми «Старт».",
    fullyCompletedTitle: "Полностью выполнено!",
    fullyCompletedHint: "Ты получил полную награду за это задание.",
    bioPrivacyWarning: "Для проверки приватность Bio в Telegram должна быть установлена на «Все».",
    errorTagMissing: "Тег не найден в имени. Добавь его и попробуй снова.",
    errorBioHidden:
      "Реферальная ссылка не найдена в Bio — убедись, что приватность Bio установлена на «Все» и ссылка там есть.",
    errorCheckFailed: "Не удалось проверить прямо сейчас — попробуй через момент.",
    errorGenericStart: "Не удалось начать этап. Попробуй снова.",
    errorGenericVerify: "Не удалось проверить этап. Попробуй снова.",
    errorTimerReset: "Условие больше не выполняется — таймер этапа сброшен. Верни условие и снова нажми «Старт».",
    rewardToast: (amount) => `+${amount} TON — этап пройден!`,
    loadError: "Не удалось загрузить особые задания.",
  },
};

export default ru;
